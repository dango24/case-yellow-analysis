...
public class OoklaImageParser extends ImageTestParser {
    ...
    private String ooklaPreLocationIdentifier;
    ...
    public double parseSpeedTest(Map<String, String> data) throws IOException {
        ...
        try {
            ...
            resultPreLocationIndex = ocrResponse.getTextAnnotations().indexOf(new WordData(ooklaPreLocationIdentifier));

            return Double.valueOf(ocrResponse.getTextAnnotations().get(resultPreLocationIndex -1).getDescription());

        } catch (Exception e) {
            throw new SpeedTestParserException("Failed to parse ookla image, " + e.getMessage(), e);
        }
    }
}
